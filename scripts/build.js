...
const staticTargets = ["index.html", "online-games", "roblox", "minecraft", "steam", "blooket", "wordle", "gear", "about", "game", "assets", "data", "sitemap.xml", "robots.txt", "manifest.webmanifest"];
...
const urls = site.pages.map((page) => `  <url><loc>https://playzonex.xyz${page.path}</loc><changefreq>${page.path === "/" ? "daily" : "weekly"}</changefreq><priority>${page.path === "/" ? "1.0" : "0.8"}</priority></url>`).join("\n");
